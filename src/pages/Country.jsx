      ...
      <div className="p-8 md:py-0 max-w-7xl mx-auto">
        {country.map((item) => (
          ...
          <div key={item.population} className="grid grid-cols-1 gap-8 md:grid-cols-2 md:place-items-center md:h-screen">
            <div>
              <img className="rounded-lg" src={item.flags.svg} alt={item.name.common} />
            </div>
            ...
            <div>
              ...
              <ul className="my-4 grid grid-cols-1 md:grid-cols-2 items-start justify-start gap-2 text-slate-700 dark:text-gray-400">
                <li>Native Name: {item.name.official}</li>
                <li>Population: {item.population.toLocaleString()}</li>
                <li>Region: {item.region}</li>
                <li>Subregion: {item.subregion}</li>
                <li>Capital: {item.capital[0]}</li>
                <li>Top Level: {item.tld}</li>
                <li>Currencies:{item.currencies[Object.keys(item.currencies)].name}</li>
                <li>Languages: {Object.values(item.languages)+``} </li>
              </ul>
